 ...
 * @DateCreated 3/12/21
 * @LastEdited 3/12/21
 * @Description Write a program with one thread (producer) taht produces items
 *              (ints). Another thread (consumer) consumes items. For
 *              communication pruposes both threads have acesss to a bounded
 *              buffer which is basically an array
 * 
 *              side note, tried to do everything in main like Pramod, however
 *              cannot use notify since it cannot be initialized for both
 *              another note: switched from synchronizing on resource to
 *              synchronizing on thread, since it wouldn't recognize either
 *              thread as owning resource when notifying the other and causing
 *              an exception. Does not seem to cause race conditions. but would
